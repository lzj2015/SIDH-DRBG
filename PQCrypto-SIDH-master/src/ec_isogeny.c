...
{ // Computes the j-invariant of a Montgomery curve with projective constant.
  ...
  // Output: j=256*(A^2-3*C^2)^3/(C^4*(A^2-4*C^2)), which is the j-invariant of the Montgomery curve B*y^2=x^3+(A/C)*x^2+x or (equivalently) j-invariant of B'*y^2=C*x^3+A*x^2+C*x.
    ...
    fp2add(t1, t1, t0);                             // t0 = t1+t1       2c^2
    fp2sub(jinv, t0, t0);                           // t0 = jinv-t0     A^2-2C^2
    fp2sub(t0, t1, t0);                             // t0 = t0-t1      A^2-3C^2
    fp2sub(t0, t1, jinv);                           // jinv = t0-t1     A^2-4C^2
    fp2sqr_mont(t1, t1);                            // t1 = t1^2        C^4
    fp2mul_mont(jinv, t1, jinv);                    // jinv = jinv*t1   (A^2-4C^2)C^4
    fp2add(t0, t0, t0);                             // t0 = t0+t0       2(A^2-3C^2)
    fp2add(t0, t0, t0);                             // t0 = t0+t0       4(A^2-3C^2)
    fp2sqr_mont(t0, t1);                            // t1 = t0^2        4^2(A^2-3C^2)^2
